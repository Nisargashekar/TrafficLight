...
enum {RED, WAIT_FOR_RED_COMPLETE, AMBER,WAIT_FOR_AMBER_COMPLETE, GREEN, WAIT_FOR_GREEN_COMPLETE,NOT_DEFINED_STATE };
uint8 TimeInState;
uint8 CurrentState;
uint8 SwitchPressCount = 0;

int main(void)
{
    CyGlobalIntEnable; /* Enable global interrupts. */

    CurrentState = NOT_DEFINED_STATE;
    ...
    for(;;)
    {
        ...
        if((SwitchPressCount >= 1)&& (Switch1_Read()== 0)){
            SwitchPressCount =0;
        ...
        else if((Switch2_Read()== 0) && (SwitchPressCount == 0)){
           LED_UpdateTimer_initVar = 1u; 
